...
def encode_token(user_id):
  payload = {
    ...
    "sub": str(user_id) # need to convert to string  or I will get on an invalid token error whewn i try ton decode
...
def token_required(f): # Wraps aroound a function "f"
  @wraps(f)
  def decoration(*args, **kwargs):
    ...
    if "Authorization" in request.headers:
      token = request.headers['Authorization'].split()[1] # headers : {"Authorization": "Bearer my_tokrn"}
